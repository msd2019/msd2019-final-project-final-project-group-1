...
Compute the Herfindahl-Hirschman Index for each team's salary. For more information, see https://en.wikipedia.org/wiki/Herfindahl_index. The index ranges from 0 to 10,000 where smaller values represent more equality in team salary distribution and larger values represent more inequality.
...
Our reproduction of summary statistics shows results that are nearly identical to the original paper. In particular, we see the same values for win percentage, with slight differences only due to rounding. Our total salary replication tends to show slightly higher values in terms of minimum, maximum, and mean. Our hypothesis for this can be seen in the notes section below. We also see slightly different values for HHI in terms of minimum and mean. This is likely due to slight differences between our dataset and the one used in the original paper. As noted in our intial step, there are occasionally incomplete data points, or teams where not every player was accounted for, which would explain the difference in mean. We are confident in our calculation of the metric given that our max HHI is identical to that in the original paper.
...
Additionally, the original paper did not include a discussion on how real world practices may affect the data. For example, baseball seasons have different periods in which a 25-man roster or 40-man roster period is allowed. Moreover, players are often designated to the minor leagues, cut, or traded to a different team in the middle of a season. We believe that this could have impacted our summary statistics results. We opted to take the data from the Lahman database verbatim, save for the removal of the incomplete data point mentioned above.

# Extension

## Extension Motivation
Given the paper's lack visualizations of the underlying data, we thought it was important to create sveral graphs to help readers understand the data, its distribution, and the various trends over time. Our other extensions were centered around better understanding the efficacy of the regression in the original paper and the parameters in it. As a result, we first wanted to see how the regression would change when it was re-run on more recent data. Next, we compared how the regressions would change with different inequality metrics, as HHI is a relatively uncommon metric. Lastly, we wanted to see how well the regression actually predicted the results and did so with different inequality metrics to see how the results changed.


## Extension Code and Analysis

Subset the data to the years 1999 to 2016, inclusive. This is because the Lahman dataset doesn't provide salary information past 2016.
...
When both models were run on the data from 1999-2016, the coefficients matched the earlier years in terms of sign. The HHI coefficient is the same in magnitude as in the early year regressions, but the coefficients for both salary and time decrease. This makes sense, as the y-values the regression is fitting to remain between 40%-70% for win percentage but total salaries of teams are increasing over time, as is the year. As a result, a million dollar increase in total salary has a comparatively smaller effect on winning than in previous years.
